...
export async function generateExceptionExcel(settlementId, farmId) {
  ...
  matchedSheet.columns = [
    ...
    { header: 'Unload MT', key: 'gross_mt', width: 12 },
    ...
    { header: 'Ticket Unload MT', key: 'ticket_net_mt', width: 14 },
  ...
  if (matchedLines.length > 0) {
    ...
    const matchedTableBody = [
      [
        ...
        { text: 'Tkt Unload MT', bold: true, fontSize: 7, alignment: 'right' },
  ...
  if (exceptions.length > 0) {
    ...
    for (const line of exceptions) {
      ...
      const leftCol = [
        ...
        `Unload: ${fmtMT(line.gross_weight_mt) ?? '—'} MT`,
