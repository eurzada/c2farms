...
async function main() {
  ...
  await prisma.userFarmRole.upsert({
    ...
    update: { modules: ['inventory', 'marketing', 'logistics'] },
    create: { user_id: user.id, farm_id: farm.id, role: 'admin', modules: ['inventory', 'marketing', 'logistics'] },
  });
  ...
  await prisma.userFarmRole.upsert({
    ...
    update: { modules: ['inventory', 'marketing', 'logistics'] },
    create: { user_id: managerUser.id, farm_id: farm.id, role: 'manager', modules: ['inventory', 'marketing', 'logistics'] },
  });
  ...
  await prisma.userFarmRole.upsert({
    ...
    update: { modules: ['inventory', 'marketing', 'logistics'] },
    create: { user_id: viewerUser.id, farm_id: farm.id, role: 'viewer', modules: ['inventory', 'marketing', 'logistics'] },
  });
  console.log(`Viewer: ${viewerUser.email}`);

  // C2 Farms team members
  const teamMembers = [
    { email: 'eric@c2farms.ca', name: 'Eric Urzada', role: 'admin' },
  ...
  for (const member of teamMembers) {
    ...
    await prisma.userFarmRole.upsert({
      ...
      update: { role: member.role, modules: ['inventory', 'marketing', 'logistics'] },
      create: { user_id: memberUser.id, farm_id: farm.id, role: member.role, modules: ['inventory', 'marketing', 'logistics'] },
    });
