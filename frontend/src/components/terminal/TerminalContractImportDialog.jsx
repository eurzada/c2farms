...
export default function TerminalContractImportDialog({ open, onClose, farmId, onImported }) {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const res = await api.post(`/api/farms/${farmId}/terminal/contracts/import-pdf`, formData);
      setExtraction(res.data.extraction);
      setUsage(res.data.usage);

      if (res.data.extraction?.contract_number) {
        try {
          const dupRes = await api.post(`/api/farms/${farmId}/terminal/contracts/import-pdf/check-duplicate`, {
  ...
  const handleConfirm = async () => {
    ...
    setError(null);
    try {
      const res = await api.post(`/api/farms/${farmId}/terminal/contracts/import-pdf/confirm`, { extraction });
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth={isPreviewStep ? 'xl' : 'sm'} fullWidth>
      <DialogTitle>Import Terminal Contract from PDF</DialogTitle>
      <DialogContent>
        ...
        {step === 'upload' && (
          <Box sx={{ textAlign: 'center', py: 4 }}>
            ...
            <Typography color="text.secondary" sx={{ mb: 3 }}>
              Upload a contract PDF (e.g. JGL, Cargill). AI will extract terms and create a Terminal Contract.
            </Typography>
        ...
        {isPreviewStep && (
          <Stack direction="row" spacing={2} sx={{ minHeight: 500 }}>
            ...
            <Box sx={{ flex: 1, minWidth: 0 }}>
              <Typography variant="subtitle2" sx={{ mb: 1 }}>Extracted Terms (will create Sale contract)</Typography>

              {duplicate && (
                <Alert severity="warning" icon={<WarningAmberIcon />} sx={{ mb: 2 }}>
                  <Typography variant="body2" fontWeight={600}>Duplicate contract detected</Typography>
                  <Typography variant="body2">
                    Contract #{duplicate.contract_number} already exists ({duplicate.buyer} — {duplicate.commodity}, {fmt(duplicate.contracted_mt)} MT).
                    Confirming will update the existing contract.
                  </Typography>
                </Alert>
              )}
                ...
                <Table size="small">
                  <TableBody>
                    {[
                      ...
                      ['Delivery', extraction.delivery_period_text || (extraction.delivery_start && extraction.delivery_end ? `${extraction.delivery_start} — ${extraction.delivery_end}` : null)],
      ...
      <DialogActions>
        {step === 'preview' && <Button onClick={handleBack} disabled={loading}>Back</Button>}
        {step === 'preview' && (
          <Button variant="contained" onClick={handleConfirm} disabled={loading} color={duplicate ? 'warning' : 'primary'}>
